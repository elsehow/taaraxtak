...
html = read_html('./test/w3techs-html/Usage Statistics and Market Share of Email Server Providers, April 2021.html')
df = utils.extract_table(html, double_table=False)
assert(len(df) == 102)

# read a page with a double table
html = read_html('./test/w3techs-html/Usage Statistics and Market Share of SSL Certificate Authorities for Websites, April 2021.html')
...
def test_weighted_gini ():
    ...
    assert(utils.weighted_gini(marketshares, population_shares) == 0.20454544413223194)
...
def test_compute_pop_weighted_gini (postgresdb):
    ...
    # US has the rest
    types.ProviderMarketshare(
        'Foo', None, 'US', 'ssl-certificate', 0.5, pd.Timestamp('2021-04-20')
    ).write_to_db(cur, conn)
